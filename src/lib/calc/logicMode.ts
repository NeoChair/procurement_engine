...
// lyBackward14가 이보다 작으면(하루 평균 5개 미만 판매) 표본이 너무 얇아 forward/backward 비율이
// 노이즈로 튀기 쉽다고 보고 추세 보정을 아예 걸지 않는다(trend=1). CAP 대신 이 최소표본 기준을 쓰는 이유는,
// CAP은 표본이 충분해도 진짜 큰 성장 신호(예: 신제품 초기 확산)까지 인위적으로 깎아버리기 때문.
const MIN_LY_BACKWARD_14D_FOR_TREND = 70;

/**
 * Default + Manual 모드에서 매뉴얼 예측치가 없는 SKU/창고에 쓰는 일 예상판매량.
 * 신제품 기준 계산(0.7/0.2/0.1)에, 작년 동일 시점 기준 "이제 막 시작되는 14일(forward)"이
 * "직전 14일(lyBackward14, backward)"보다 얼마나 늘었는지를 나타내는 추세를 곱해 계절성을 반영한다.
 * 7일보다 표본을 늘려 노이즈(프로모션/이벤트성 스파이크 등)에 덜 흔들리면서도, 단기 재고 부족을
 * 유발할 수 있는 근시일 수요 램프업 신호는 28일보다 덜 희석되도록 14일로 잡았다.
