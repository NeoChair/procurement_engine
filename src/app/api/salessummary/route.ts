...
const SUMMARY_QUERY = `
    ...
    SELECT
        ...
        ISNULL(SUM(CASE WHEN p.WRHS_CD = 14361 THEN p.QTY END), 0) AS CA1_PRODUCTION_PLAN_QTY,
        ISNULL(SUM(CASE WHEN p.WRHS_CD = 14630 THEN p.QTY END), 0) AS CA_PRODUCTION_PLAN_QTY,
        ISNULL(SUM(CASE WHEN p.WRHS_CD = 14631 THEN p.QTY END), 0) AS CA2_PRODUCTION_PLAN_QTY,
        ISNULL(SUM(CASE WHEN p.WRHS_CD = 14632 THEN p.QTY END), 0) AS GA_PRODUCTION_PLAN_QTY,
        ISNULL(SUM(CASE WHEN p.WRHS_CD = 14633 THEN p.QTY END), 0) AS GA2_PRODUCTION_PLAN_QTY,
        ISNULL(SUM(CASE WHEN p.WRHS_CD = 14634 THEN p.QTY END), 0) AS NJ_PRODUCTION_PLAN_QTY,
        ISNULL(SUM(CASE WHEN p.WRHS_CD = 14635 THEN p.QTY END), 0) AS SC_PRODUCTION_PLAN_QTY,
        ISNULL(SUM(CASE WHEN p.WRHS_CD = 14636 THEN p.QTY END), 0) AS TX_PRODUCTION_PLAN_QTY,
        ISNULL(SUM(CASE WHEN p.WRHS_CD = 0     THEN p.QTY END), 0) AS WF_PRODUCTION_PLAN_QTY
    ...
    LEFT JOIN [HGBC].[RPA].[TB_SHIPPING_PLAN_STOCK] p
        ...
        AND (p.ETD IS NULL OR p.ETD = '' OR p.ETD BETWEEN @etdStart AND @etdEnd)
